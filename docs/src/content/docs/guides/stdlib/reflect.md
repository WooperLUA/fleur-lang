...
| :--- | :--- | :--- |
| `has(struct, key)` | `Boolean` | Returns `true` if the `struct` instance has a property or method named `key`. |
| `get(struct, key)` | `Any` or `Null` | Returns the value of the property `key` from the `struct` instance, or `null`. |
| `set(struct, key, val)` | `Null` | Sets the property `key` of the `struct` instance to `val`. Requires the struct to be mutable. |
